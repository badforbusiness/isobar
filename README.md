...
2. **Concentration Constraints Deferred for Quantum**  
   Concentration constraints ($\sum_{i \in \mathcal{C}} X_i / \sum X \le \text{max\_share}$) are implemented in the classical SLSQP solver, but deferred for the quantum solver in Phase 1. Adding inequality constraints to a QUBO requires introducing slack variables and quadratic penalty terms with extra Lagrange multipliers, creating a complex parameter-tuning problem that would obscure the core tractability comparison between classical and quantum algorithms. The architecture fully supports concentration constraints in schema and classical implementation for Phase 2 integration.
