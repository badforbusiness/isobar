...
5. **8-Bit Binary Expansion for QUBO**  
   Each continuous exposure $X_i$ is mapped to $B = 8$ binary variables across its feasible range $[\text{EAD-min},i, \text{EAD-max},i]$, offering $2^8 = 256$ discrete steps ($\approx £350\text{K}$ resolution per step). Boundary constraints are satisfied **by construction** because binary linear combinations cannot yield values outside $[\text{EAD-min}, \text{EAD-max}]$, eliminating the need for boundary penalty terms in the QUBO matrix. The resulting QUBO problem requires $N \times B = 50 \times 8 = 400$ binary variables, well within Simulated Annealing capacity.
